...
## INICIO PRUEBAS LOCALES ##
import os
if not os.getenv("DBTYPE"):
    import logging
    import sys
    #import locale
...
def construir_prompt(intencion, pregunta, contexto, conocimiento_extra=" "):
    ...
    if params:
        logg(f"construir_prompt: Usando Plantilla: [{intencion}] " )
    if len(params) >= 1:
        entidad = params[0]
    if len(params) >= 2:
        inst = params[1]
    ...
    if len(params) >= 3:
        ...
        if rule:
            rules += "\n" + rule
    plantilla = header + task + instruction + knowledge + ahora + question + rules
